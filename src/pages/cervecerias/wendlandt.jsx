...
export default function WendlandtPage() {
  ...
  const [mostrarCervezas, setMostrarCervezas] = useState(true); // Estado para mostrar cervezas o bebidas
    ...
    const fetchData = async () => {
      try {
        const [cervezasResponse, bebidasResponse] = await Promise.all([
          fetch("http://localhost:3000/cervezas/1"), // Aquí se usa el ID 1 para Wendlandt
          fetch("http://localhost:3000/bebidas/1"), // Aquí también el ID para bebidas
        ]);
        ...
        <div className="flex gap-2 p-2 bg-transparent sticky top-0 z-10 max-w-4xl mx-auto">
          <button
            onClick={() => {
              ...
              setMostrarCervezas(true); // Muestra la sección de cervezas
            }}
          ...
          <button
            onClick={() => {
              ...
              setMostrarCervezas(false); // Muestra la sección de bebidas
            }}
            className={`flex-1 py-3 px-4 text-white border-none rounded-lg cursor-pointer text-xl font-bold transition-colors ${
              !mostrarCervezas
                ? "bg-[#8B6642]"
